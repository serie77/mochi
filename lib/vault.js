...
export function recordPps(pps) {
  const h = readJson("pps-history.json", []);
  const last = h[h.length - 1];
  if (last && Date.now() - last.t < 3600_000) return;
  h.push({ t: Date.now(), pps: String(pps) });
  writeJson("pps-history.json", h.slice(-24 * 30));
}
...
export async function stakersMap() {
  ...
  if (!stakingCa) return {};
  const c = client();
  const cache = readJson("stakers.json", null);
