...
async function secret() {
  ...
  const f = await readJson("auth-secret.json", null);
  if (f?.secret) return (cachedSecret = f.secret);
  const s = crypto.randomBytes(32).toString("hex");
  await writeJson("auth-secret.json", { secret: s });
...
export async function signToken(address) {
...
export async function verifyToken(token) {
  ...
  const want = crypto.createHmac("sha256", await secret()).update(payload).digest("base64url");
  if (want.length !== sig.length || !crypto.timingSafeEqual(Buffer.from(want), Buffer.from(sig))) return null;
  try {
    const { a, e } = JSON.parse(Buffer.from(payload, "base64url").toString("utf8"));
    if (!a || e < Date.now()) return null;
    // lowercase everywhere: the ledger, votes, and chain snapshots all key by lowercase address
    const v = checksum(a);
