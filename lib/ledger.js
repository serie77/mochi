...
// wallet keys are always lowercase; sessions, chain snapshots, and gifts may arrive checksummed
const wkey = (a) => String(a || "").toLowerCase();
...
export function handleOfAddr(s, addr) {
  addr = wkey(addr);
  for (const [h, a] of Object.entries(s.handles)) if (wkey(a) === addr) return h;
  return null;
}
...
export async function ribbonsOf(addr) {
  return (await readJson("ledger.json", { balances: {} })).balances[wkey(addr)] || 0;
}
export async function inventoryOf(addr) {
  return (await readJson("inventory.json", {}))[wkey(addr)] || [];
}
