...
export function tally(p, weights) {
  ...
  for (const [a, cid] of Object.entries(p.votes)) {
    ...
    t[cid] += weights ? weights[String(a).toLowerCase()] || 0 : 1;
  }
  return t;
}
...
export async function runEpoch({ now = Date.now(), force = false } = {}) {
  ...
  if (c.tokenCa) {
    try {
      ...
      weights = {};
      for (const h of list) weights[String(h.address).toLowerCase()] = Number(h.balance) / 1e18;
    } catch (e) {
    ...
    if (vaultCfg().stakingCa) {
      try {
        ...
        weights ||= {};
        for (const [a0, amt] of Object.entries(stakers)) {
          const a = String(a0).toLowerCase();
